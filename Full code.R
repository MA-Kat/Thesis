...
### Mine press releases from the internet (code ran on March 7th)
dir.create("../PRhtmls")
...
AS_full2 <- anti_join(AS_full, AS_outliers, by = "term")
AS_full2 <- left_join(AS_full2, countrycharacteristics, by = c("country" = "Country"))
AS_full2$new <- if_else(as.double(AS_full2$`Accession year`) < 2004, "no", "yes")
...
AS_full2_all <- AS_full2[! (AS_full2$presmonth < -89 |AS_full2$presmonth >76),]

### Results for equal weight for all words
rdd_all_avg <- rdd_data(AS_full2_all$perc, AS_full2_all$presmonth, cutpoint=0)
reg_all_avg <- rdd_reg_lm(rdd_object = rdd_all_avg, order=1)
print(reg_all_avg)
summary(reg_all_avg)

### Results and graph with for equal weight per country
rdd_all_avg <- AS_full2_all %>% group_by(country, presmonth) %>% summarise_at("perc", mean)
...
AS_full22 <- filter(AS_full2_all, AS_full2_all$council_configuration != "Unclassified")

rdd_all_avg <- AS_full22 %>% group_by(country, presmonth) %>% summarise_at("perc", mean)
rdd_all_avg <- rdd_data(rdd_all_avg$perc, rdd_all_avg$presmonth, cutpoint=0)
reg_all_avg <- rdd_reg_lm(rdd_object = rdd_all_avg, order=1)
print(reg_all_avg)
plot(reg_all_avg, xlab="Number of Months before/after the Presidency", ylab="Average Percentage")
...
rdd_all_avg <- filter(AS_full2_all, AS_full2_all$country %in% countriesB) %>% group_by(presmonth, country) %>% summarise_at("perc", mean)
rdd_all_avg <- rdd_data(rdd_all_avg$perc, rdd_all_avg$presmonth, cutpoint=0)
reg_all_avg <- rdd_reg_lm(rdd_object = rdd_all_avg, order=1)
print(reg_all_avg)
summary(reg_all_avg)

rdd_all_avg <- filter(AS_full2_all, AS_full2_all$country %in% countriesJ) %>% group_by(presmonth, country) %>% summarise_at("perc", mean)
rdd_all_avg <- rdd_data(rdd_all_avg$perc, rdd_all_avg$presmonth, cutpoint=0)
reg_all_avg <- rdd_reg_lm(rdd_object = rdd_all_avg, order=1)
print(reg_all_avg)
summary(reg_all_avg)
...
rdd_all_avg <- filter(AS_full2, AS_full2$country %in% countriesjan) %>% group_by(presmonth, country) %>% summarise_at("perc", mean)
rdd_all_avg <- rdd_data(rdd_all_avg$perc, rdd_all_avg$presmonth, cutpoint=0)
reg_all_avg <- rdd_reg_lm(rdd_object = rdd_all_avg, order=1)
print(reg_all_avg)
summary(reg_all_avg)
...
rdd_all_avg <- filter(AS_full2, AS_full2$country %in% countriescrisis) %>% group_by(presmonth, country) %>% summarise_at("perc", mean)
rdd_all_avg <- rdd_data(rdd_all_avg$perc, rdd_all_avg$presmonth, cutpoint=0)
reg_all_avg <- rdd_reg_lm(rdd_object = rdd_all_avg, order=1)
print(reg_all_avg)
plot(reg_all_avg)
...
createRDDs_country <- function(MSs) {
  rdd_country <- filter(AS_full2, AS_full2$country == MSs)
...
createRDDs_country2 <- function(MSs) {
  rdd_country <- filter(AS_full2, AS_full2$country == MSs & AS_full2$council_configuration != "Unclassified")
  rdd_object=rdd_data(rdd_country$perc, rdd_country$presmonth, cutpoint=0)
}
...
council_configurations <- AS_full2 %>% count(council_configuration)
...
createRDDs_council <- function(councilconf) {
  rdd_council <- filter(AS_full2_all, AS_full2_all$council_configuration == councilconf) %>% group_by(presmonth, country) %>% summarise_at("perc", mean)
...
createRDDs_council2 <- function(councilconf) {
  rdd_council <- filter(AS_full2_all, !(AS_full2_all$country %in% countriesJ) & AS_full2_all$council_configuration == councilconf)
  rdd_object=rdd_data(rdd_council$perc, rdd_council$presmonth, cutpoint=0)
}
...
createRDDs_location <- function(x) {
  rdd_council <- filter(AS_full2, AS_full2$Location == x)
  rdd_object=rdd_data(rdd_council$perc, rdd_council$presmonth, cutpoint=0)
}
...
months2 <- as.data.frame(months) %>%  rownames_to_column()
months2$rowname <- as.double(months2$rowname) - 83
months2$Juncker <- if_else(months2$rowname>=0, 1, 0)
summary(lm(months2$n ~ months2$Juncker))
rdd_months <- rdd_reg_lm(rdd_object = rdd_data(months2$n, months2$rowname, cutpoint=0), order=1)
summary(rdd_months)
plot(rdd_months ,
...
resultstable2 <- left_join(countrycharacteristics, resultstable, by = c("Country" = "country"))
summary(lm(resultstable2$percest ~ resultstable2$Population))
summary(lm(resultstable2$percest ~ resultstable2$GDP))
summary(lm(resultstable2$percest ~ resultstable2$`Accession year`))
